...
Page({
  ...
  setupPlaySong(id) {
    ...
    // 4. audioContext的四个监听事件 (监听只要第一次就可以了)
  ...
  onSliderChanging: throttle(function(event) {
    ...
    // 2.根据当前的值, 计算出对应的时间
    const currentTime = value / 100 * this.data.durationTime
    // 调用setDa 页面会重新刷新, 频繁滑动,current会不断更新, 所以在滑动过程中 onSliderChanging要做节流 
  ...
  changeNewSong(isNext = true) {
    ...
    switch (this.data.playModeIndex) {
      ...
      // 让case为单曲循环时, 点击上下曲也按顺序, case 1不写break, 会穿透,可以共享下case 0的逻辑, 但是当前歌快结束时, audioContext.onEnded 和 modeIndex就需要做判断, 让它继续播放当前歌曲
      case 1:
      case 0: // 顺序播放
      ...
      case 2: // 随机播放
        ...
        console.log(index);
        if (this.data.playSongIndex === index) {
          console.log("重复了");
          index = Math.floor(Math.random() * length)
          // 重新调用这个函数前需要再随机一个index, 因为下面playStore要保存最新索引值
          this.changeNewSong()
        }
        break
    }
  ...
  getPlaySongInfosHandler({ playSongList, playSongIndex }) {
    ...
    // playSongIndex可能为0, 0也需要设置进去, 所以不能直接写playSongIndex
